...
export class HanaCountBuilder<
	TSession extends HanaSession<any, any, any>,
> extends SQL<number> implements Promise<number>, SQLWrapper<number> {
	private sql: SQL<number>;

	static override readonly [entityKind]: string = 'HanaCountBuilder';
	[Symbol.toStringTag] = 'HanaCountBuilder';
	...
	constructor(
		...
		super(HanaCountBuilder.buildEmbeddedCount(params.source, params.filters).queryChunks);

		this.mapWith(Number);

		this.session = params.session;

		this.sql = HanaCountBuilder.buildCount(
			params.source,
			params.filters,
		);
	}
